...
def staff_info(newpage2):
    ...
    def ok1():
        ...
        des=ID.get().upper()
        a = -1
        if des == 'COMPUTER OPERATOR':
            a=1000
        elif des == 'LAB ASSISTANT':
            a=1001

        if len(inputName) != ' ' and len(dept) != ' ' and Password.get() == Confirm_Password.get() and '@.gmail.com' in phone.get():
            with open("Data.csv", "a") as dataFile:
                csvWriter = csv.writer(dataFile)
                csvWriter.writerow([inputName, Department.get().upper(), des, Password.get(), phone.get(), a])
                dataFile.close()
            print("done csv")
    ...
    options=['COMPUTER OPERATOR','LAB ASSISTANT']
    ...
    b1=ttk.Combobox(popupwindow,value=depart,width=27,text=Department).grid(row=2, column=1)
